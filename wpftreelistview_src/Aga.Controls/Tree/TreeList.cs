...
namespace Aga.Controls.Tree
{
	public class TreeList: ListView
	{
		...
		{
            var IModel = parent.Tag as ITreeModel;
            if (IModel != null)
                return IModel.GetChildren(parent.Tag);
			else if (Model != null)
		...
		private bool HasChildren(TreeNode parent)
		{
            if (parent == Root)
                return true;
            else if (parent.Tag is ITreeModel)
                return (parent.Tag as ITreeModel).HasChildren(parent.Tag);
            else if (Model != null)
